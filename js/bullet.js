...
$.Bullet = function (opt) {
  ...
  this.isHoming = !!opt.isHoming; // se é um tiro inteligente
  this.isExplosive = !!opt.isExplosive; // se é um tiro explosivo
  this.homingStrength = opt.homingStrength || 0.15; // quanto o tiro "vira" por frame
  this.explosiveRadius = opt.explosiveRadius || 40; // raio da explosão (nerf)
...
$.Bullet.prototype.update = function (i) {
  /*==============================================================================
  Homing (tiro inteligente)
  ==============================================================================*/
  if (this.isHoming) {
    let target = $.util.getNearestEnemy(this.x, this.y);
    if (target) {
      let dx = target.x - this.x;
      let dy = target.y - this.y;
      let desired = Math.atan2(dy, dx);
      // interpolação angular para suavizar
      let diff = ((desired - this.direction + $.tau + $.pi) % $.tau) - $.pi;
      this.direction += diff * this.homingStrength;
    }
  }
