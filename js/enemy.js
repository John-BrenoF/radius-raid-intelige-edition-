...
$.Enemy.prototype.update = function (i) {
  ...
  // Adicionar variável global para controle de boss vivo
  if (this.isBoss && this.life > 0) {
    $.bossAlive = true;
  }
};
...
$.Enemy.prototype.receiveDamage = function (i, val) {
  ...
  if (this.isBoss && val > 1000 && Math.random() < 0.75) return; // imune a 75% das bombas do player
  this.life -= val;
  this.hitFlag = 10;
  if (this.life <= 0) {
    if (this.inView) {
    ...
    $.enemies.splice(i, 1);
    if (this.isBoss && this.life <= 0) {
      $.hero.specials.healCount = Math.min($.hero.specials.healCount + 3, 4); // máximo 4
      $.bossActive = false;
    }
  }
  if (this.isBoss) {
    this.shotsTaken = (this.shotsTaken || 0) + 1;
  }
};
